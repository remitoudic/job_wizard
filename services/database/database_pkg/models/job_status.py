...
class JobStatus(SQLModel, table=True):
    ...
    job_id: str = Field(
        primary_key=True, index=True, description="Unique identifier for the job"
    )
    status: str = Field(
        description="Current status (extracting, generating, completed, error)"
    )
    ...
    updated_at: datetime = Field(
        default_factory=datetime.utcnow, description="Timestamp of last status update"
    )
