...
with engine.connect() as conn:
    try:
        conn.execute(text("ALTER TABLE user ADD COLUMN preferred_language VARCHAR DEFAULT 'en'"))
        conn.commit()
...
try:
    ...
    pg_engine = create_engine("postgresql://postgres:postgres@localhost:5432/job_wizard")
