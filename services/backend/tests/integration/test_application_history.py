...
def test_application_history_tracking(
    ...
    update_data = {"status": "interview", "notes": "First interview scheduled"}
    resp = client.patch(
        f"/api/application/{app_id}", json=update_data, headers=auth_headers
    )
    assert resp.status_code == 200
    ...
    resp = client.patch(
        f"/api/application/{app_id}", json={"status": "finish"}, headers=auth_headers
    )
    assert resp.status_code == 200
