...
def create_app_for_user(
    ...
    app = Application(
        ...
        cover_letter_final={"model": "test", "timestamp": "2025-01-01", "body": "Dear..."},
    )
    session.add(app)
    session.commit()
    session.refresh(app)
...
class TestCheckDuplicateNoMatch:
    ...
    def test_different_url_no_match(
        self, client: TestClient, auth_headers: dict, test_session: Session, test_user: User
    ):
...
class TestCheckDuplicateOtherUser:
    ...
    def test_other_user_app_not_matched(
        self, client: TestClient, auth_headers: dict, test_session: Session, other_user: User
    ):
