...
def test_delete_application_success(
    ...
    response = client.delete(f"/api/application/{app_id}", headers=auth_headers)
...
def test_delete_application_not_found(client: TestClient, auth_headers: dict):
    ...
    response = client.delete("/api/application/999999", headers=auth_headers)

    assert response.status_code == 404
    assert "Application not found" in response.json()["detail"]


def test_delete_other_user_application(
    client: TestClient, auth_headers: dict, test_session: Session
):
    ...
    response = client.delete(f"/api/application/{other_app.id}", headers=auth_headers)
