...
def create_app_for_user(
    ...
    """Create an application + job description for a user."""
...
class TestCheckDuplicateNoMatch:
    def test_no_applications_at_all(self, client: TestClient, auth_headers: dict):
        resp = client.get(
            "/api/application/check-duplicate?job_url=https://example.com/job/123",
            headers=auth_headers,
        )
        assert resp.status_code == 200
        assert resp.json() == {"is_duplicate": False, "existing_application": None}

    def test_different_url_no_match(
        self,
        client: TestClient,
        auth_headers: dict,
...
class TestCheckDuplicateExactMatch:
    def test_exact_url_match(
        ...
        resp = client.get(
            f"/api/application/check-duplicate?job_url={url}",
            headers=auth_headers,
        )
        assert resp.status_code == 200
        data = resp.json()
...
class TestCheckDuplicateUrlNormalization:
    def test_www_vs_non_www(
        ...
        resp = client.get(
            "/api/application/check-duplicate?job_url=https://www.example.com/job/123",
            headers=auth_headers,
        )
        assert resp.status_code == 200
...
class TestCheckDuplicateLinkedIn:
    ...
    COLLECTIONS_URL = (
        "https://www.linkedin.com/jobs/collections/recommended/?currentJobId=4386770393"
    )

    def test_stored_view_searched_collections(
        self,
        client: TestClient,
        auth_headers: dict,
        test_session: Session,
        ...
        resp = client.get(
            f"/api/application/check-duplicate?job_url={self.COLLECTIONS_URL}",
            headers=auth_headers,
        )
        assert resp.status_code == 200
        assert resp.json()["is_duplicate"] is True

    def test_stored_collections_searched_view(
        self,
        client: TestClient,
        auth_headers: dict,
        test_session: Session,
        ...
        resp = client.get(
            f"/api/application/check-duplicate?job_url={self.VIEW_URL}",
            headers=auth_headers,
        )
        assert resp.status_code == 200
        assert resp.json()["is_duplicate"] is True

    def test_view_url_without_www(
        self,
        client: TestClient,
        auth_headers: dict,
        test_session: Session,
        ...
        create_app_for_user(
            test_session, test_user, "https://www.linkedin.com/jobs/view/4386770393/"
        )
        resp = client.get(
            "/api/application/check-duplicate?job_url=https://linkedin.com/jobs/view/4386770393/",
            headers=auth_headers,
        )
        assert resp.status_code == 200
        assert resp.json()["is_duplicate"] is True
...
class TestCheckDuplicateOtherUser:
    def test_other_user_app_not_matched(
        self,
        client: TestClient,
        auth_headers: dict,
        test_session: Session,
