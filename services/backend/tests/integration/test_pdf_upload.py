...
def create_dummy_pdf_bytes():
    ...
    c.drawString(
        100, 690, "Experience: 5 years of experience in building web applications."
    )
    c.save()
    buffer.seek(0)
    return buffer.read()
...
def test_upload_invalid_file():
    response = client.post(
        "/api/upload-context", files={"file": ("test.txt", b"plain text", "text/plain")}
    )
...
def test_upload_image_regression():
    ...
    response = client.post(
        "/api/upload-image", files={"file": ("test.gif", img_content, "image/gif")}
    )
    assert response.status_code == 200
    data = response.json()
    assert "filename" in data
