...
async def save_application(request: SaveApplicationRequest, session: SessionDep, current_user: CurrentUser):
    try:
        ...
        statement = select(DBJobDescription).where(DBJobDescription.url.in_(list(url_variants)))
        ...
        existing_app_statement = select(Application).where(Application.user_id == current_user.id, Application.job_description_id == job_description.id)
