<script lang="ts">
	...
	function resolveTemplate(): string {
		if (selectedTemplate.startsWith('modern')) {
			return modernLayout === 'single' ? 'modern_single' : 'modern';
		}
	...
	async function setModernLayout(layout: 'single' | 'two') {
		modernLayout = layout;
		await fetchPreview();
	}
	...
	async function goToGenerate() {
		...
		step = 'generate';
		// Kick off the first preview after templates load
		await fetchPreview();
	}
	...
	async function handleGenerate() {
		...
		error = '';
		try {
			const actualTemplate = resolveTemplate();
	...
	function isStepAvailable(s: string) {
		...
		if (s === 'review') return cvData.contact.name !== '' || cvData.experiences.length > 0;
		if (s === 'generate')
			return (cvData.contact.name !== '' || cvData.experiences.length > 0) && step !== 'upload';
		return false;
	}
...
<div class="min-h-screen py-16 px-4 bg-gradient-to-b from-white to-slate-50">
	<div class="max-w-4xl mx-auto">
			...
			<div class="flex items-start gap-4">
				...
				<div>
					...
					<p class="text-lg text-white/90 leading-relaxed">
						Upload your old CV, review the parsed data, and download a fresh, modern PDF.
					</p>
				</div>
			</div>
		</div>
				...
				<div class="flex items-center gap-2">
					<button
						...
						class="w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold transition-all duration-300
							...
							: (s.key === 'upload' && (step === 'review' || step === 'generate')) ||
					...
					<span class="text-sm font-medium text-slate-600 hidden sm:inline">{s.label}</span>
					{#if i < 2}
			...
			>
				{#if loading}
					<div class="flex flex-col items-center gap-4">
						...
						></div>
						<p class="text-lg font-semibold text-[#0F172A]">Parsing your CV…</p>
				...
				{:else}
					<div class="flex flex-col items-center gap-4">
						...
						<div>
							<p class="text-xl font-bold text-[#0F172A] mb-1">Drop your CV here</p>
							<p class="text-slate-500">or click to browse • PDF only</p>
						</div>
						<label
						...
						>
							Choose File
							<input type="file" accept=".pdf" class="hidden" on:change={handleFileInput} />
						</label>
					</div>
				{/if}
			</div>
			...
			<div class="space-y-6">
				...
				<div class="bg-white rounded-xl border border-[#E2E8F0] shadow-sm p-6">
					...
					<div class="grid md:grid-cols-2 gap-4">
						<div>
							<label for="full-name" class="block text-sm font-medium text-slate-600 mb-1"
								>Full Name</label
							>
							<input
								type="text"
						...
						<div>
							<label for="phone" class="block text-sm font-medium text-slate-600 mb-1">Phone</label>
							<input
								type="tel"
								id="phone"
						...
						<div>
							<label for="linkedin" class="block text-sm font-medium text-slate-600 mb-1"
								>LinkedIn</label
							>
							<input
								type="url"
						...
						<div class="md:col-span-2">
							<label for="address" class="block text-sm font-medium text-slate-600 mb-1"
								>Address</label
							>
							<input
								type="text"
				...
				<!-- Summary -->
				<div class="bg-white rounded-xl border border-[#E2E8F0] shadow-sm p-6">
					<h2 class="text-xl font-bold text-[#0F172A] mb-4">Professional Summary</h2>
					<textarea
				...
				<div class="bg-white rounded-xl border border-[#E2E8F0] shadow-sm p-6">
					...
					{#each cvData.experiences as exp, i}
						<div class="border border-slate-200 rounded-lg p-4 mb-4 relative group">
							<button
							...
							<div class="grid md:grid-cols-2 gap-3 mb-3">
								<div>
									<label for="exp-title-{i}" class="block text-xs font-medium text-slate-500 mb-1"
										>Job Title</label
									>
									<input
										type="text"
								...
								<div>
									<label for="exp-company-{i}" class="block text-xs font-medium text-slate-500 mb-1"
										>Company</label
									>
									<input
										type="text"
								...
								<div>
									<label for="exp-start-{i}" class="block text-xs font-medium text-slate-500 mb-1"
										>Start Date</label
									>
									<input
										type="text"
								...
								<div>
									<label for="exp-end-{i}" class="block text-xs font-medium text-slate-500 mb-1"
										>End Date</label
									>
									<input
										type="text"
							...
							<div>
								<label for="exp-desc-{i}" class="block text-xs font-medium text-slate-500 mb-1"
									>Description</label
								>
								<textarea
									id="exp-desc-{i}"
					...
					{#if cvData.experiences.length === 0}
						<p class="text-slate-400 text-sm italic">No experiences — click "+ Add" above.</p>
					{/if}
				</div>
				...
				<div class="bg-white rounded-xl border border-[#E2E8F0] shadow-sm p-6">
					...
					{#each cvData.education as edu, i}
						<div class="border border-slate-200 rounded-lg p-4 mb-4 relative group">
							...
							<div class="grid md:grid-cols-2 gap-3">
								<div>
									<label for="edu-degree-{i}" class="block text-xs font-medium text-slate-500 mb-1"
										>Degree</label
									>
									<input
										type="text"
								...
								<div>
									<label
										...
										class="block text-xs font-medium text-slate-500 mb-1">Institution</label
									>
									<input
										type="text"
								...
								<div>
									<label for="edu-start-{i}" class="block text-xs font-medium text-slate-500 mb-1"
										>Start Date</label
									>
									<input
										type="text"
								...
								<div>
									<label for="edu-end-{i}" class="block text-xs font-medium text-slate-500 mb-1"
										>End Date</label
									>
									<input
										type="text"
					...
					{#if cvData.education.length === 0}
						<p class="text-slate-400 text-sm italic">No education entries — click "+ Add" above.</p>
					{/if}
				</div>

				<!-- Skills -->
				<div class="bg-white rounded-xl border border-[#E2E8F0] shadow-sm p-6">
					<h2 class="text-xl font-bold text-[#0F172A] mb-4">Skills</h2>
					<div class="flex flex-wrap gap-2 mb-3">
						{#each cvData.skills as skill, i}
							...
							>
								{skill}
								<button on:click={() => removeSkill(i)} class="hover:text-red-500 transition-colors"
									>×</button
								>
							</span>
						{/each}
				...
				<!-- Languages -->
				<div class="bg-white rounded-xl border border-[#E2E8F0] shadow-sm p-6">
					<h2 class="text-xl font-bold text-[#0F172A] mb-4">Languages</h2>
					<div class="flex flex-wrap gap-2 mb-3">
						{#each cvData.languages as lang, i}
							...
							>
								...
								<button
									...
									class="hover:text-red-500 transition-colors">×</button
								>
							</span>
						{/each}
					</div>
		...
		{:else if step === 'generate'}
			<!-- Expand to max-w-6xl for the split-panel -->
			<div class="space-y-6 -mx-4 sm:-mx-8 lg:-mx-16 xl:-mx-32 px-4 sm:px-8 lg:px-16 xl:px-32">
				<!-- Split panel: picker left, preview right -->
				<div class="flex flex-col lg:flex-row gap-6 items-start">
					<!-- ── LEFT: Template Picker ─────────────────────────── -->
					<div class="w-full lg:w-[38%] flex-shrink-0">
						<div class="bg-white rounded-xl border border-[#E2E8F0] shadow-sm p-6">
							<h2 class="text-xl font-bold text-[#0F172A] mb-5">Choose a Template</h2>

							<!-- Template cards -->
							<div class="flex flex-col gap-3">
								{#each templates.filter((t) => t.name !== 'modern_single') as t}
									<button
										on:click={() => selectTemplate(t.name)}
										class="border-2 rounded-xl p-4 text-left transition-all duration-200 cursor-pointer
                                            {selectedTemplate.startsWith(t.name)
											? 'border-[#0369A1] bg-[#EFF6FF] shadow-md'
											: 'border-slate-200 hover:border-[#0369A1]/40 hover:shadow-sm'}"
									>
										<div class="flex items-center gap-3">
											<div
												class="w-10 h-10 rounded-lg flex items-center justify-center font-bold text-sm flex-shrink-0 transition-colors duration-200
                                                    {selectedTemplate.startsWith(t.name)
											...
											{#if selectedTemplate.startsWith(t.name)}
												<svg
													xmlns="http://www.w3.org/2000/svg"
													class="h-5 w-5 text-[#0369A1] flex-shrink-0"
													viewBox="0 0 20 20"
													fill="currentColor"
													aria-hidden="true"
												>
								...
								{#if templates.length === 0 && !loading}
									<p class="text-slate-400 italic text-sm">No templates found.</p>
								{/if}
							</div>

							<!-- Layout Selector if Modern is chosen -->
							{#if selectedTemplate.startsWith('modern')}
								<div class="mt-5 p-4 border border-[#BFDBFE] bg-[#EFF6FF] rounded-xl">
									<h3 class="font-semibold text-sm text-[#0F172A] mb-3">Layout Style</h3>
									<div class="flex gap-3">
										<!-- Single Column -->
										<button
											on:click={() => setModernLayout('single')}
											class="flex-1 border-2 rounded-lg p-3 text-left transition-all duration-200 cursor-pointer
                                                {modernLayout === 'single'
												? 'border-[#0369A1] bg-white shadow-sm'
												: 'border-slate-200 bg-white/60 hover:border-slate-300'}"
										>
											<div class="flex flex-col items-center gap-1.5">
												<div
													class="w-8 h-10 border border-slate-300 rounded flex flex-col gap-0.5 p-1"
												>
													<div class="w-full h-2 bg-slate-300 rounded-sm"></div>
													<div class="w-full h-1 bg-slate-200 rounded-sm"></div>
													<div class="w-full h-3 bg-slate-200 rounded-sm"></div>
												</div>
												<span class="text-xs font-medium text-slate-700">Single</span>
											</div>
										</button>
										<!-- Two Column -->
										<button
											on:click={() => setModernLayout('two')}
											class="flex-1 border-2 rounded-lg p-3 text-left transition-all duration-200 cursor-pointer
                                                {modernLayout === 'two'
												? 'border-[#0369A1] bg-white shadow-sm'
												: 'border-slate-200 bg-white/60 hover:border-slate-300'}"
										>
											<div class="flex flex-col items-center gap-1.5">
												<div class="w-8 h-10 border border-slate-300 rounded flex gap-0.5 p-1">
													<div class="w-1/3 h-full bg-slate-200 rounded-sm"></div>
													<div class="w-2/3 flex flex-col gap-0.5">
														<div class="w-full h-2 bg-slate-300 rounded-sm"></div>
														<div class="w-full h-3 bg-slate-200 rounded-sm"></div>
													</div>
												</div>
												<span class="text-xs font-medium text-slate-700">Two Col</span>
											</div>
										</button>
									</div>
									<p class="text-xs text-slate-500 mt-2">
										{#if modernLayout === 'single'}
											ATS-friendly. Best for long CVs.
										{:else}
							...
							<!-- Mobile: Show/Hide Preview toggle -->
							<button
								on:click={() => (previewVisible = !previewVisible)}
								class="mt-4 w-full flex items-center justify-center gap-2 py-2.5 border border-slate-200 rounded-lg text-sm font-medium text-slate-600 hover:bg-slate-50 transition-colors duration-200 cursor-pointer lg:hidden"
					...
					<div
						id="cv-preview-panel"
						class="w-full lg:flex-1 {previewVisible ? 'block' : 'hidden'} lg:block"
						aria-live="polite"
						aria-label="CV template preview"
					>
						<!-- Header label -->
						<div class="flex items-center justify-between mb-2 px-1">
							...
							{#if previewLoading}
								<span class="text-xs text-slate-400 flex items-center gap-1.5">
									<span
										class="inline-block w-3 h-3 border-2 border-slate-300 border-t-[#0369A1] rounded-full animate-spin"
									></span>
									Updating…
								</span>
							{/if}
						...
						<!-- Preview frame -->
						<div
							class="relative bg-white rounded-xl border border-[#E2E8F0] shadow-lg overflow-hidden"
							style="height: 680px;"
						>
							{#if previewLoading && !previewHtml}
								<!-- Initial skeleton -->
								<div class="absolute inset-0 flex flex-col gap-4 p-8 animate-pulse">
									<div class="h-14 bg-slate-200 rounded-lg w-full"></div>
									...
									<div class="flex gap-4 flex-1">
										<div class="w-1/3 flex flex-col gap-3">
											...
											<div class="mt-3 h-3 bg-slate-200 rounded w-2/3"></div>
											<div class="h-3 bg-slate-100 rounded w-full"></div>
										</div>
										<div class="flex-1 flex flex-col gap-3">
											...
											<div class="h-3 bg-slate-100 rounded w-full"></div>
											<div class="h-3 bg-slate-100 rounded w-4/5"></div>
											<div class="mt-3 h-3 bg-slate-200 rounded w-1/3"></div>
											<div class="h-3 bg-slate-100 rounded w-full"></div>
						...
						<p class="text-xs text-slate-400 mt-2 px-1 text-center">
							This is an HTML preview — your downloaded PDF may have minor rendering differences.
						</p>
					</div>
				</div>
				...
				<div class="flex justify-between pt-2">
					<button
						on:click={goBack}
						class="px-5 py-2.5 border border-slate-300 text-slate-600 rounded-lg hover:bg-slate-50 transition-colors duration-200 font-medium cursor-pointer"
					>
						← Back to Edit
					</button>
					<button
						...
						disabled={loading}
						class="px-8 py-3 bg-gradient-to-r from-[#0369A1] to-[#0284C7] text-white rounded-lg font-bold shadow-lg hover:shadow-xl transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 cursor-pointer"
					>
						{#if loading}
							<div
