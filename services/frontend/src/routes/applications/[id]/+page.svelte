		...
		{:else if error}
			<div class="bg-red-50 border border-red-200 text-red-700 p-4 rounded-xl text-center">
				...
				<button
					...
					class="mt-4 text-red-800 font-bold underline">Try Again</button
				>
			</div>
		{:else if details}
			<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
				...
				<div class="lg:col-span-2 space-y-6">
					<section
						class="bg-white rounded-xl border border-[#E2E8F0] shadow-sm overflow-hidden flex flex-col h-[calc(100vh-12rem)]"
					>
						<div
							class="p-4 bg-slate-50 border-b border-slate-100 flex items-center justify-between"
						>
							<h2 class="text-sm font-bold uppercase tracking-wider text-slate-500">
								Cover Letter Body
							</h2>
							<span class="text-xs text-slate-400"
								>Autosave enabled - Changes are saved automatically</span
							>
						</div>
						<textarea
				...
				<div class="space-y-6">
					...
					<section class="bg-white rounded-xl border border-[#E2E8F0] shadow-sm p-6 space-y-4">
						...
						<div
							class="relative pl-6 space-y-6 before:content-[''] before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-100"
						>
							{#each history as item}
								<div class="relative">
									<!-- Dot -->
									<div
										class="absolute -left-[20px] top-1.5 w-3 h-3 rounded-full border-2 border-white bg-blue-500 shadow-sm z-10"
									></div>
									...
									<div class="flex flex-col">
										<div class="flex items-center gap-2">
											...
											{/if}
											<span
												class="px-2 py-0.5 rounded-full text-[10px] font-bold uppercase {getStatusColor(
													item.new_status
												)}"
											>
												{item.new_status}
										...
										>
										{#if item.notes}
											<p class="text-xs text-slate-600 mt-1 italic italic-slate-500">
												"{item.notes}"
											</p>
										{/if}
									</div>
