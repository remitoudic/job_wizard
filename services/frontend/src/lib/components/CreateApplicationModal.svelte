<script lang="ts">
	...
	async function handleUpdateExisting() {
		...
		try {
			...
			if (cover_letter_body && cover_letter_body !== (existing.cover_letter_body || ''))
				updateData.cover_letter_body = cover_letter_body;
	...
	>
		...
		>
			...
			<div class="p-6 overflow-y-auto space-y-6">
				{#if error}
					<div
						class="p-4 bg-red-50 border border-red-100 text-red-600 rounded-xl text-sm flex items-start gap-3"
					>
						<svg
							xmlns="http://www.w3.org/2000/svg"
				...
				{#if showDuplicateWarning && duplicateCheck?.existing_application}
					{@const existing = duplicateCheck.existing_application}
					<div
						class="p-5 bg-amber-50 border border-amber-200 rounded-xl"
						transition:fade={{ duration: 200 }}
					>
						<div class="flex items-start gap-3 mb-4">
							...
							<div>
								...
								<p class="text-sm text-amber-700 mt-1">
									The URL you entered matches an existing application.
								</p>
							</div>
						</div>
											...
											: existing.status === 'waiting'
												...
												: existing.status === 'accepted'
													...
													: existing.status === 'refused'
														...
														: 'bg-gray-100 text-gray-700'}">{existing.status}</span
								>
							</div>
							<p class="font-bold text-slate-900">{existing.job_title}</p>
						...
						<div class="flex flex-wrap gap-3">
							<button
								on:click={handleUpdateExisting}
								disabled={isUpdating}
								class="bg-[#0369A1] hover:bg-[#0284C7] text-white px-6 py-2.5 rounded-xl font-bold transition-all shadow-lg hover:shadow-xl disabled:opacity-50 disabled:shadow-none flex items-center gap-2"
							>
								{#if isUpdating}<svg
										...
										/></svg
									>Updating...{:else}Update Existing{/if}</button
							>
							<button
								on:click={handleForceCreate}
								disabled={isSaving}
								class="px-6 py-2.5 rounded-xl font-semibold border border-slate-300 text-slate-700 hover:bg-slate-50 transition-colors"
								>Create New Anyway</button
							>
							<button
								on:click={handleCancelDuplicate}
								class="px-6 py-2.5 rounded-xl font-semibold text-slate-500 hover:text-slate-700 hover:bg-slate-100 transition-colors"
								>Cancel</button
							>
						</div>
					</div>
				{:else}
					<div class="grid grid-cols-1 md:grid-cols-2 gap-5">
						<div class="space-y-1.5">
							<label for="company" class="text-sm font-semibold text-slate-700 ml-1"
								>Company *</label
							><input
								id="company"
								type="text"
						...
						<div class="space-y-1.5">
							<label for="job_title" class="text-sm font-semibold text-slate-700 ml-1"
								>Job Title *</label
							><input
								id="job_title"
								type="text"
					...
					<div class="grid grid-cols-1 md:grid-cols-2 gap-5">
						...
						<div class="space-y-1.5">
							<label for="status" class="text-sm font-semibold text-slate-700 ml-1"
							...
							<div class="relative">
								...
								>
									<option value="applied">Applied</option><option value="waiting">Waiting</option
									><option value="interview">Interview</option><option value="accepted"
										>Accepted</option
									><option value="refused">Refused</option><option value="finish">Finished</option
									></select
								>
								<div
					...
					<div class="space-y-1.5">
						<label for="notes" class="text-sm font-semibold text-slate-700 ml-1"
					...
					<div class="space-y-1.5">
						<label for="cover_letter" class="text-sm font-semibold text-slate-700 ml-1"
			...
			<div
				class="px-6 py-5 border-t border-slate-100 flex items-center justify-end gap-3 bg-slate-50/50"
			>
				<button
					on:click={close}
					...
					>
						{#if isSaving}<svg
								...
								/><path
									...
									d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"
								/></svg
							>Checking...{:else}Create Application{/if}</button
					>
				{/if}
			</div>
		</div>
